...
mostCommonResultCodes () {
	...
	do
		echo ${resultCodeField[$i]} ${resultCodeCountField[$i]} >> mfile
		for ((j=0;j<=${#ipField[$i][@]};j++))
		do
			echo ${ipField["$i_$j"]} >> mfile
	    done
	done;

	sort -n -k 2 -r mfile > sfile
	echo "most common result codes"
	if [ -z $n ]; then
		n=`wc -l < sfile`
	fi
	head -$n sfile
	...
	unset resultCodeField
	unset resultCodeCountField
